...
// --- Matching function (unique ID resolver) ---
function isSameRecipe(a, b) {
  return (
    (a.id && b.id && a.id === b.id) ||
    (a.idMeal && b.idMeal && a.idMeal === b.idMeal) ||
    (a.strMeal && b.strMeal && a.strMeal === b.strMeal) ||
...
export default function RecipeCard({
  ...
  useEffect(() => {
    try {
      ...
      const exists = stored.some((r) => isSameRecipe(r, recipe));
      setIsSaved(exists);
    } catch {
      setIsSaved(false);
    }
  ...
  function handleHeartClick(e) {
    ...
    // Saved page: unsave by index
    if (onUnsave) {
      onUnsave();
      return;
    }

    // Home page: save/unsave in localStorage
    const stored = JSON.parse(localStorage.getItem(LS_KEY)) || [];

    if (isSaved) {
      const next = stored.filter((r) => !isSameRecipe(r, recipe));
